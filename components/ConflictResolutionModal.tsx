...
interface ConflictResolutionModalProps {
  ...
  onResolve: (resolution: 'keep_new' | 'cancel' | 'keep_both') => void;
}

export const ConflictResolutionModal: React.FC<ConflictResolutionModalProps> = ({ conflict, onResolve }) => {
  if (!conflict) return null;

  const { newlySelectedTag, conflictingTags } = conflict;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 transition-opacity duration-300" aria-modal="true" role="dialog">
      <div className="bg-white dark:bg-bunker-900 rounded-lg shadow-2xl p-6 w-full max-w-md transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div className="mt-2">
              <p className="text-sm text-bunker-600 dark:text-bunker-300">
                You've selected <strong className="text-blue-500">"{newlySelectedTag.label}"</strong>, which conflicts with the already selected {conflictingTags.length > 1 ? 'tags' : 'tag'}: <strong className="text-red-500">{conflictingTags.map(t => `"${t.label}"`).join(', ')}</strong>.
              </p>
              <p className="text-sm text-bunker-600 dark:text-bunker-300 mt-2">
                Please choose which tags you'd like to keep.
              </p>
            </div>
          </div>
        </div>
        <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-3 sm:gap-3">
          <button
            type="button"
            className="w-full inline-flex justify-center rounded-md border border-bunker-300 dark:border-bunker-600 shadow-sm px-4 py-2 bg-white dark:bg-bunker-800 text-base font-medium text-bunker-700 dark:text-bunker-200 hover:bg-bunker-50 dark:hover:bg-bunker-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-bunker-900 focus:ring-bunker-500 sm:text-sm transition-colors"
          ...
          <button
            type="button"
            className="mt-3 w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-bunker-900 focus:ring-blue-500 sm:mt-0 sm:text-sm transition-colors"
